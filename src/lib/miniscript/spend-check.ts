...
export function evaluateSpendPaths(opts: {
  ...
  const coinHeight = Math.max(0, Math.floor(Number(opts.coinHeight) || 0));
  ...
  const stages = slots.map((slot, i) => {
    ...
    const lockOpen = slot.delay <= 0 || conf >= slot.delay;
    const blocksLeft = lockOpen ? 0 : Math.max(0, slot.delay - conf);
    const opensAt = slot.delay <= 0 ? tip : coinHeight > 0 ? coinHeight + slot.delay - 1 : tip + slot.delay;
    return {
      index: slot.index,
  ...
  return {
    ...
    anyLater: stages.some((s) => s.canSign && !s.lockOpen),
  };
}
