...
- **Watch-only wallet** — Descriptor tab → **Wallet**: the current policy as a watch-only wallet (no signing). With Core connected, descriptor check and UTXO scan run on load. Refresh derives receive/change via Core and lists coins via Electrum/Fulcrum. Balance, used addresses, per-address UTXOs with sum. Units: Bitcoin / satoshi icons / Auto (each amount on its own; below 0.1 BTC as sats). BTC always eight decimals, locale separators. Coin list at the bottom always BTC.
