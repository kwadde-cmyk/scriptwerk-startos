...
- **Stages** — k-of-n, relative timelock (`older` / CSV) or absolute block height (`after` / CLTV), keys that must always co-sign (`A + (B or C)`), several recovery stages
...
- **Import / Export** — descriptor, miniscript, BSMS, Scriptwerk JSON, `scriptwerk.keys.txt` (names), BIP-388 for Ledger and BitBox (QR, file, USB). Files carry key names; QR stays comment-free. A public descriptor that does not map 1:1 onto Scriptwerk stages stays watch-only (frozen original); Taproot (`tr` / musig) is rejected.
- **Saved policies** — named copies in this browser (same name overwrites). Load restores stages, keys and expert flags. A new design is titled **New / unsaved**; editing a loaded policy shows **modified** until you save.
...
- **Watch-only wallet** — Descriptor tab → **Wallet**: the current policy as a watch-only wallet (no signing). With Core connected, descriptor check and UTXO scan run on load. Changing keys or stages (new addresses) drops the previous wallet's balance immediately — coins only display when the scan checksum matches this descriptor. Refresh derives receive/change via Core and lists coins via Electrum/Fulcrum. Coin list shows amount, age (confirmations) and per-stage spendability (green open lock / red closed lock + remaining blocks). BIP-329 JSONL import/export for address and coin labels (Sparrow, Labelbase, Electrum). Balance, used addresses, per-address UTXOs with sum. Units: Bitcoin / satoshi icons / Auto (each amount on its own; below 0.1 BTC as sats). BTC always eight decimals, locale separators. Coin amounts always BTC.
...
- **StartOS** — wrapper in `deploy/startos`, sideload the `.s9pk` or Community Registry. Bitcoin Core, Fulcrum and Electrs are optional; Scriptwerk starts without them. Fulcrum is preferred when both Electrum servers are installed.
...
- **Stufen** — k-von-n, relatives Timelock (`older` / CSV) oder absolute Blockhöhe (`after` / CLTV), Keys die immer mitunterschreiben (`A + (B oder C)`), mehrere Recovery-Stufen
...
- **Import / Export** — Descriptor, Miniscript, BSMS, Scriptwerk-JSON, `scriptwerk.keys.txt` (Namen), BIP-388 für Ledger und BitBox (QR, Datei, USB). Dateien tragen Key-Namen mit; QR bleibt ohne Kommentare. Ein öffentlicher Descriptor, der nicht 1:1 auf Scriptwerk-Stufen passt, bleibt Watch-only (Original eingefroren); Taproot (`tr` / musig) wird abgelehnt.
...
- **Watch-only-Wallet** — Descriptor-Tab → **Wallet**: die aktuelle Policy als Beobachtungs-Wallet (kein Signing). Mit Core verbunden laufen Descriptor-Check und UTXO-Scan beim Laden. Aktualisieren leitet Empfang/Wechsel über Core ab und listet Coins über Electrum/Fulcrum. Coin-Liste: Betrag, Alter und Spendbarkeit je Stufe (grün offenes Schloss / rot geschlossenes Schloss + Restblöcke). BIP-329 JSONL Import/Export für Adress- und Coin-Labels (Sparrow, Labelbase, Electrum). Saldo, benutzte Adressen, UTXOs je Adresse mit Summe. Einheiten: Bitcoin-/Satoshi-Icons / Auto (jeder Betrag für sich; unter 0,1 BTC als Sats). BTC immer acht Nachkommastellen, lokale Trennzeichen. Coins-Liste unten immer BTC.
- **Ausgabepfad-Check** — Descriptor-Tab → **Prüfen**: Geräte antippen, die du dabei hast. Zeigt, welche Stufe jetzt spendbar ist, welche Keys fehlen, und wie viele CSV-Blöcke noch fehlen (Höhe von Core/Electrum oder letzter UTXO-Scan).
...
- **StartOS** — Wrapper in `deploy/startos`, Sideload der `.s9pk` oder Community-Registry. Bitcoin Core, Fulcrum und Electrs sind optional; Scriptwerk startet ohne sie. Fulcrum hat Vorrang, wenn beide Electrum-Server installiert sind.
